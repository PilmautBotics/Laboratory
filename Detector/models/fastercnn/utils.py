...
def boxes_to_transformation_targets(ground_truth_boxes, anchors_or_proposals):
    ...
    device = anchors_or_proposals.device
    ground_truth_boxes = ground_truth_boxes.to(device)
    
    # return zero like if no boxe inside of anchors and ground truth boxes
    if ground_truth_boxes.numel() == 0:
        return torch.zeros_like(anchors_or_proposals)
    ...
    widths = torch.max(widths, torch.tensor(epsilon, dtype=torch.float32, device=device))  # Ensure no zero widths
    heights = torch.max(heights, torch.tensor(epsilon, dtype=torch.float32, device=device))  # Ensure no zero heights
    gt_widths = torch.max(gt_widths, torch.tensor(epsilon, dtype=torch.float32, device=device))  # Ensure no zero gt_widths
    gt_heights = torch.max(gt_heights, torch.tensor(epsilon, dtype=torch.float32, device=device))  # Ensure no zero gt_heights
